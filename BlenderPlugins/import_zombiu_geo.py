...
def read_positions(data: bytes, header: GeoHeader, scale: float) -> List[Tuple[float, float, float]]:
    points = []
    for i in range(header.position_count):
        x, y, z = _f32x3(data, POSITION_OFFSET + i * 12)
        # ZombiU weapon meshes in the samples are Y-forward-ish. Blender is Z-up,
        # so rotate into a more natural viewport orientation on import.
    ...
    while offset >= 0:
        if offset + 20 <= len(data):
            ...
            if (
                ...
                # The render-cache blocks store their header fields as
                # little-endian longs, but the u16 payload itself is big-endian.
                words = [
...
def find_render_index_block(
    ...
    for block in iter_render_index_blocks(data):
        ...
        # The geometry block is the one whose payload indexes the point array.
        # Its paired UV/render-vertex block has the same marker but indexes a
        # larger domain, so it fails the point-count range check above.
...
def best_faces_for_run(
    ...
    # Some late chunks are not plain triangle lists. If list triples barely share
    # edges with their neighbors, decode the same atom stream as a strip.
...
def iter_index_runs(data: bytes, max_index: int) -> Iterable[Tuple[int, List[int]]]:
    # The PC files that I exported and have used unaligned blocks, and the useful index/display
    # streams live in the tail. Earlier numeric tables can mimic index buffers.
    scan_start = int(len(data) * TAIL_SCAN_FRACTION)
    for parity in (0, 1):
        run_start = None
        words: List[int] = []
...
def import_geo(filepath: str, scale: float, flip_uv_v: bool) -> List[bpy.types.Object]:
    ...
    objects = [
        build_mesh_object(blender_object_name(base_name, part), points, uvs, normals, part, header)
        for part in parts
    ]
...
class IMPORT_OT_zombiu_geo(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        try:
            objects = import_geo(self.filepath, self.scale, self.flip_uv_v)
        except Exception as exc:
            self.report({"ERROR"}, str(exc))
            return {"CANCELLED"}

        face_count = sum(len(obj.data.polygons) for obj in objects)
        self.report({"INFO"}, f"Imported {len(objects)} GEO part(s), {face_count} faces")
        return {"FINISHED"}
