...
def parse_tdt_texture_data(data: bytes, path: str = "<memory>") -> TdtTexture:
...
def parse_tdt_texture(path: str) -> TdtTexture:
    with open(path, "rb") as handle:
        return parse_tdt_texture_data(handle.read(), path)
...
def decode_tdt_top_mip_rgba_data(data: bytes, info: TdtTexture | None = None) -> Tuple[int, int, bytes]:
    ...
    level = info.levels[0]
    payload = data[level.data_offset:level.data_offset + level.data_size]
...
def decode_tdt_top_mip_rgba(path: str, info: TdtTexture | None = None) -> Tuple[int, int, bytes]:
    with open(path, "rb") as handle:
        data = handle.read()
    return decode_tdt_top_mip_rgba_data(data, info or parse_tdt_texture_data(data, path))
...
def png_bytes_from_rgba(width: int, height: int, rgba: bytes) -> bytes:
    rows = bytearray()
    row_size = width * 4
...
def write_png_rgba(path: str, width: int, height: int, rgba: bytes) -> None:
    ...
    with open(path, "wb") as handle:
        handle.write(png_bytes_from_rgba(width, height, rgba))


def convert_tdt_data_to_png_bytes(data: bytes) -> Tuple[int, int, bytes]:
    width, height, rgba = decode_tdt_top_mip_rgba_data(data)
    return width, height, png_bytes_from_rgba(width, height, rgba)


def write_tdt_data_as_png(data: bytes, png_path: str) -> str:
    width, height, rgba = decode_tdt_top_mip_rgba_data(data)
